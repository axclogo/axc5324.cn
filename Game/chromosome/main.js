...
function loadResource() {
	...
	loadGameData();
	USE_NATIVE_SOUND || (IS_NATIVE_ANDROID ? (createjs.Sound.registMySound("do", 0), createjs.Sound.registMySound("re", 2), createjs.Sound.registMySound("me", 4), createjs.Sound.registMySound("fa", 6), createjs.Sound.registMySound("so", 8), createjs.Sound.registMySound("la", 10), createjs.Sound.registMySound("silenttail", 12), qp_l.loadFile({
		id: "sound",
		src: RES_DIR + "audio/all.mp3"
	})) : (createjs.Sound.alternateExtensions = ["ogg"], qp_l.installPlugin(createjs.Sound), qp_l.loadManifest({
		path: RES_DIR + "audio/",
		manifest: [{
			src: "1.mp3",
			id: "do"
...
function qp_q(a, b, c) {
	...
	for(d = 0; d < qp_k.m_changeditems.length; d++) {
		a = (qp_k.m_changeditems[d].indexj + qp_k.m_changeditems[d].indexi + 1) * qp_c, createjs.Tween.get(qp_k.m_changeditems[d].img).to({
			alpha: 1
		}, a).call(function() {
			this.graphics.clear();
...
function qp_r(a) {
	for(a = 0; a < qp_b; a++) {
		for(var b = 0; b < qp_a; b++) {
			var c = (a + b + 1) * qp_c,
				d = qp_k.m_allitems[a * qp_a + b].img;
			d.alpha = 0;
